...
| #   | Area              | Decision                                                                                                                                                     | Phase |
| --- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| 1   | Bill numbering    | Indian financial year (Apr–Mar). Format `P1-2026-27-0001`. Each login has its own independent sequence, allocated atomically server-side.                    | 4     |
| 2   | Online orders     | On verified payment, auto-generate the bill under house partner code **P1** (configurable in Settings).                                                      | 4–5   |
...
| 13  | Users             | 5 internal accounts: P1, P2, P3 (partners, full access), S1, S2 (staff, restricted). RBAC enforced on the backend.                                           | 1     |
...
- **Bill numbering (decisions 1 & 2).** `bill_sequences` keyed by
  `(user_code, fiscal_year_label)` where the label is e.g. `"2026-27"`. Online
  orders allocate under the configured house code (default `P1`); the bill row
  records both `user_code` and the (nullable) `created_by_user_id`.
