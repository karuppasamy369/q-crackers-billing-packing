...
| 14  | Tracking          | Cryptographically-random tokens. No order IDs or PII in tracking URLs. Links do not expire in V1; partners can revoke. Rate limited. **Phase 6 shipped a first tracking page at `/track/<reference>` keyed by the order's existing 24-byte random reference (no PII, rate-limited); dedicated revocable tokens + `tracking_events` remain Phase 7.**                                                                                   | 7     |
